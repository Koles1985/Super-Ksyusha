...
public class World {
    ...
    private void genereteLevel(){
        float y = Platform.PLATFORM_HEIGHT / 2;
        float maxJampHeight = Bob.BOB_JUMP_VELOCITY * Bob.BOB_JUMP_VELOCITY /
                (2 * -gravity.y);

        while(y < WORLD_HEIGHT - WORLD_HEIGHT / 2){
            ...
            y += (maxJampHeight - 0.5f);
            y -= rand.nextFloat() * (maxJampHeight / 3);
        }
    ...
    private void updatePlatforms(float deltatime){
        ...
        for(int i = 0; i < len; i++){
            ...
            if(platform.state == Platform.PLATFORM_STATE_PULVERIZING
                && platform.stateTime > Platform.PLATFORM_PULVERRIZING_TIME){
    ...
    private void checkSquirrelCollision(){
        int len = squirrels.size();
        for(int i = 0; i < len; i++){
            Squirrel squirrel = squirrels.get(i);
    ...
    private void checkItemCollision(){
        ...
        for(int i = 0; i < len; i++){
            Coin coin = coins.get(i);
            if(OverlapTester.overlapRectangles(coin.bounds, bob.bounds)){
                coins.remove(coin);
                len = coins.size();
                listener.coin();
    ...
    private void checkCastleCollision(){
